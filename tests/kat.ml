...
let test_kat_data i rng_data () =
  ...
  let actual = match (name.digits, name.word_size) with
    | (Two,  ThirtyTwo) -> R2x32.rand_R nrounds (ctr |> Array.map I32.of_string) (key |> Array.map I32.of_string)
                           |> Array.map I32.to_string
    | (Two,  SixtyFour) -> R2x64.rand_R nrounds (ctr |> Array.map I64.of_string) (key |> Array.map I64.of_string)
                           |> Array.map I64.to_string
    | (Four, ThirtyTwo) -> R4x32.rand_R nrounds (ctr |> Array.map I32.of_string) (key |> Array.map I32.of_string)
                           |> Array.map I32.to_string
    | (Four, SixtyFour) -> R4x64.rand_R nrounds (ctr |> Array.map I64.of_string) (key |> Array.map I64.of_string)
                           |> Array.map I64.to_string
  in
